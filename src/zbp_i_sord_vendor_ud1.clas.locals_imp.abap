...
*** Declaration
    TYPES: BEGIN OF ty_buffer_ord.
             INCLUDE TYPE zsord_vendor_ud1 AS data.
...
*** Create data
    ...
    LOOP AT entities ASSIGNING FIELD-SYMBOL(<lfs_entities>).
      IF <lfs_entities>-VendorId IS INITIAL. "check for vendor id
        ...
        ls_failed-%msg = new_message( id       = 'ZRAP1'
                                      number   = 000
...
*** Get last saved sales order
        SELECT MAX( sord ) FROM zsord_vendor_ud1 INTO @DATA(lv_max_sord).
...
DATA ls_failed LIKE LINE OF reported-vendor.
    ...
    LOOP AT entities ASSIGNING FIELD-SYMBOL(<lfs_new_data>).
      ...
      SELECT SINGLE * FROM zsord_vendor_ud1 WHERE sord = @<lfs_new_data>-Sord
...
*** Get already saved data
    LOOP AT keys ASSIGNING FIELD-SYMBOL(<lfs_keys>).
      SELECT * FROM zsord_vendor_ud1 WHERE sord = @<lfs_keys>-Sord INTO TABLE @DATA(lt_result_tmp) .
...
*** Enable/Disable rows based on Overall Status
    ...
    LOOP AT keys ASSIGNING FIELD-SYMBOL(<lfs_keys>).
      SELECT SINGLE * FROM zsord_vendor_ud1 WHERE sord = @<lfs_keys>-Sord INTO @DATA(ls_result_tmp) .
...
CLASS lsc_ZI_SORD_VENDOR_UD1 DEFINITION INHERITING FROM cl_abap_behavior_saver.
  PROTECTED SECTION.
...
CLASS lsc_ZI_SORD_VENDOR_UD1 IMPLEMENTATION.
